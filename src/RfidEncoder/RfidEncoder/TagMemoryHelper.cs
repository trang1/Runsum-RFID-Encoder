...
{
    public static class TagMemoryHelper
    {
        /// <summary>
        ...
        private static ushort[] ReadLargeTagMemoryWithoutZeroLengthSupport(Reader reader, Gen2.Bank bank, TagFilter filter)
        {
            lock (new Object())
            {
                ushort[] data = null;
                ...
                while (false == isAllDataReceived)
                {
                    try
                    {
                        ...
                        dataTemp.AddRange(((ushort[])reader.ExecuteTagOp(op, filter)));
                    ...
                    catch (Exception ex)
                    {
                        try
                        {
                            if (ex is FAULT_GEN2_PROTOCOL_MEMORY_OVERRUN_BAD_PC_Exception || (-1 != ex.Message.IndexOf("Non-specific reader error")) || (-1 != ex.Message.IndexOf("General Tag Error")) || (-1 != ex.Message.IndexOf("Tag data access failed")))
                            {
                                ...
                                ReadTagMemoryWordByWord(reader, bank, filter, ref data);
        ...
        private static void ReadLargeTagMemory(Reader reader, Gen2.Bank bank, TagFilter filter, ref ushort[] data)
        {
            lock (new Object())
            {
                List<ushort> dataTemp = new List<ushort>();
                ...
                while (false == isAllDataReceived)
                {
                    ...
                    {
                        op = new Gen2.ReadData(bank, startaddress, Convert.ToByte(words));
                        ushort[] tempDataReceived = (ushort[])reader.ExecuteTagOp(op, filter);
        ...
        private static void ReadTagMemoryWordByWord(Reader reader, Gen2.Bank bank, TagFilter filter, ref ushort[] data)
        {
            lock (new Object())
            {
                List<ushort> dataTemp = new List<ushort>();
        ...
        private static ushort[] ReadMemoryM6eVariants(Reader reader, Gen2.Bank bank, TagFilter filter)
        {
            try
            {
                // Read tag memory with zero as start address and lenght of words to read as zero
                TagOp op = new Gen2.ReadData(bank, 0, 0);
                var MemData = (ushort[])reader.ExecuteTagOp(op, filter);
                ...
                if (MemData.Length < 64)
                {
                    ...
                    ReadTagMemoryWordByWord(reader, bank, filter, ref MemData);
                }
                else
                {
                    // If data read is more then 64 words then perform read with start address as 64 words
                    // and length as zero
                    ReadLargeTagMemory(reader, bank, filter, ref MemData);
                }
            ...
            catch (Exception ex)
            {
                if ((ex is FAULT_PROTOCOL_BIT_DECODING_FAILED_Exception) || ((ex is FAULT_NO_TAGS_FOUND_Exception)))
                {
                    ...
                    return ReadLargeTagMemoryWithoutZeroLengthSupport(reader, bank, filter);
                }
                else
                {
                    throw;
